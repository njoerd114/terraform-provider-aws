...
* `engine_mode` - (Optional) The database engine mode. Valid values: `global` (only valid for Aurora MySQL 1.21 and earlier), `multimaster`, `parallelquery`, `provisioned`, `serverless`, `serverlessv2`. Defaults to: `provisioned`. See the [RDS User Guide](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/aurora-serverless.html) for limitations when using `serverless` or `serverlessv2`.
...
* `v2_scaling_configuration`- (Optional) Nested attribute with scaling properties. Only valid when `engine_mode`is set to `serverlessv2`. More details below.
...
### v2_scaling_configuration Argument Reference

~> **NOTE:** `v2_scaling_configuration` configuration is only valid when `engine_mode` is set to `serverlessv2`.
...
* `min_capacity` - (Optional) The minimum capacity for an Aurora DB cluster in `serverlessv2` DB engine mode. The minimum capacity must be lesser than or equal to the maximum capacity. Valid capacity values are in a range of `0.5` up to `128` in steps of `0.5`.
